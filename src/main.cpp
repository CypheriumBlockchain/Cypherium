...
#if defined(NDEBUG)
# error "Cypherium cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xcd0da61c966d0db80ce5f5ca7217ffc3fde855a8e91f224289cf7c072fb4c3a0");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Cypherium: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Cypherium Signed Message:\n";
...
{
    // Cypherium: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Cypherium: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Cypherium: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1518485123, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "The Times 02 12 18 Bitcoin is in a recession";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 10 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04049c27ca06718da8fff17b8775b91b8b575fdd8a66edb0e38a57bda31d681bc30b96074cfc000b529def6fd680e3a649127725fec42101d59a781d73898ad459") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1518485123;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 5002456;
        ...
        assert(block.hashMerkleRoot == uint256("0x064cf4ba425d62bc055b024edb4afb668d811b9c3a33bc2bdc29d9da9390629c"));
...
unsigned char pchMessageStart[4] = { 0x98, 0x5e, 0x59, 0x07 }; // Cypherium: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CypheriumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CypheriumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CypheriumMiner(CWallet *pwallet)
{
    printf("CypheriumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cypherium-miner");
    ...
    try { loop {
        ...
        printf("Running CypheriumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CypheriumMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CypheriumMiner, pwallet));
}
